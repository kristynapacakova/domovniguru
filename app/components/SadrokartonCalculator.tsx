...
export default function SadrokartonCalculator() {
  const [delka, setDelka]     = useState("4");
  const [vyska, setVyska]     = useState("2.5");
  const [deska, setDeska]     = useState("3.0");
  const [rezerva, setRezerva] = useState("10");

  const n = (v: string) => parseFloat(v.replace(",", ".")) || 0;
  const plocha    = n(delka) * n(vyska);
  const plochaRez = plocha * (1 + n(rezerva) / 100);
  const pocet     = n(deska) > 0 ? Math.ceil(plochaRez / n(deska)) : 0;
  const copyText  = `Potřebujete ${pocet} desek sádrokartonu na ${plochaRez.toFixed(2)} m² (s ${n(rezerva)} % rezervou). Spočíteno na DomovniGuru.cz`;
    ...
    <div style={{ background: bg, borderRadius:"16px", padding:"28px 24px", border:`1px solid ${border}`, fontFamily:"inherit" }}>
      <div style={{ marginBottom:"20px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"4px" }}>
          ...
          <h3 style={{ margin:0, fontSize:"18px", fontWeight:700, color:"#2a2a28" }}>Kalkulačka sádrokartonu</h3>
        </div>
        <p style={{ margin:0, fontSize:"13px", color:muted }}>Spočítej počet SDK desek na příčku nebo podhled.</p>
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(140px, 1fr))", gap:"14px", marginBottom:"18px" }}>
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Délka plochy (m)</label>
        ...
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Výška plochy (m)</label>
        ...
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Plocha jedné desky (m²)</label>
          <input type="number" min="0" step="0.1" value={deska}
            onChange={(e) => setDeska(e.target.value)}
            style={inputCss}
            onFocus={e=>(e.target.style.borderColor=accent)}
            onBlur={e=>(e.target.style.borderColor=border)} />
          <span style={{ fontSize:"11px", color:muted, marginTop:"4px" }}>standard: 1,25 × 2,0 m = 2,5 m²</span>
        </div>
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Rezerva na řezy (%)</label>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"14px" }}>
        ...
        <div style={{ background:"#fff", border:`1px solid ${border}`, borderRadius:"10px", padding:"12px 14px" }}>
          <div style={{ fontSize:"10px", color:muted, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:"4px" }}>S rezervou</div>
      ...
      <div style={{ background:accent, borderRadius:"12px", padding:"20px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"12px" }}>
        <div>
          ...
          <div style={{ fontSize:"32px", fontWeight:800, color:"#fff", lineHeight:1.1 }}>{pocet} desek SDK</div>
        </div>
      ...
      <div style={{ marginTop:"12px", background:"#fffbf0", border:"1px solid #e8d890", borderRadius:"8px", padding:"10px 14px", fontSize:"12px", color:"#7a6a20", display:"flex", gap:"8px" }}>
        <span>💡</span><span>Standardní deska 12,5 mm má rozměr 1,25 × 2,0 m = 2,5 m². Pro podhled nebo zvýšené zatížení volte 15 mm. Přidejte vždy 10 % na řezy.</span>
      </div>
    </div>
  );
}
