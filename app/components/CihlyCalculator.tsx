...
export default function CihlyCalculator() {
  const [plocha, setPlocha]             = useState("10");
  const [cihel_na_m2, setCihel_na_m2]   = useState("65");
  const [rezerva, setRezerva]           = useState("5");

  const n = (v: string) => parseFloat(v.replace(",", ".")) || 0;

  const pocet = Math.ceil(n(plocha) * n(cihel_na_m2) * (1 + n(rezerva) / 100));

  const copyText = `Potřebujete ${pocet} kusů cihel na ${n(plocha)} m² zdi (${n(cihel_na_m2)} ks/m², rezerva ${n(rezerva)} %). Spočíteno na DomovniGuru.cz`;
    ...
    <div style={{ background: bg, borderRadius:"16px", padding:"28px 24px", border:`1px solid ${border}`, fontFamily:"inherit" }}>
      <div style={{ marginBottom:"20px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"4px" }}>
          ...
          <h3 style={{ margin:0, fontSize:"18px", fontWeight:700, color:"#2a2a28" }}>Kalkulačka cihel</h3>
        </div>
        <p style={{ margin:0, fontSize:"13px", color:muted }}>Spočítej počet cihel nebo tvárnic na zeď nebo příčku.</p>
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(140px, 1fr))", gap:"14px", marginBottom:"18px" }}>
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Plocha zdi (m²)</label>
        ...
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Cihel na 1 m² zdi</label>
          <input type="number" min="0" step="1" value={cihel_na_m2}
            onChange={(e) => setCihel_na_m2(e.target.value)}
            style={inputCss}
            onFocus={e=>(e.target.style.borderColor=accent)}
            onBlur={e=>(e.target.style.borderColor=border)} />
          <span style={{ fontSize:"11px", color:"#9a9a90", marginTop:"4px" }}>standard DF (plocha): 65 ks / příčkovka: 35 ks</span>
        </div>
        <div style={{ display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"14px" }}>
        ...
        <div style={{ background:"#fff", border:`1px solid ${border}`, borderRadius:"10px", padding:"12px 14px" }}>
          <div style={{ fontSize:"10px", color:muted, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:"4px" }}>Cihel celkem</div>
      ...
      <div style={{ background:accent, borderRadius:"12px", padding:"20px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"12px" }}>
        <div>
          ...
          <div style={{ fontSize:"32px", fontWeight:800, color:"#fff", lineHeight:1.1 }}>{pocet} kusů cihel</div>
          <div style={{ fontSize:"12px", color:"#b0b0a0", marginTop:"4px" }}>{n(cihel_na_m2)} ks/m² · rezerva {n(rezerva)} %</div>
        </div>
      ...
      <div style={{ marginTop:"12px", background:"#fffbf0", border:"1px solid #e8d890", borderRadius:"8px", padding:"10px 14px", fontSize:"12px", color:"#7a6a20", display:"flex", gap:"8px" }}>
        <span>💡</span><span>Standardní plná cihla DF (240×115×71 mm): ~65 ks/m² pro jednovrstvou zeď. Příčkovka (115×71×240 mm): ~35 ks/m². Vždy přidejte 5 % rezervu.</span>
      </div>
    </div>
  );
}
