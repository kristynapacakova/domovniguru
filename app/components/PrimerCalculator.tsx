...
export default function PrimerCalculator() {
  const [plocha, setPlocha]         = useState("40");
  const [vydatnost, setVydatnost]   = useState("8");
  const [vrstvy, setVrstvy]         = useState("1");

  const n = (v: string) => parseFloat(v.replace(",", ".")) || 0;
  const litry  = (n(plocha) * n(vrstvy)) / n(vydatnost);
  const litryZ = Math.ceil(litry * 10) / 10;
  const copyText = `Potřebujete ${litryZ.toFixed(1)} litrů penetrace na ${n(plocha)} m². Spočíteno na DomovniGuru.cz`;
    ...
    <div style={{ background: bg, borderRadius:"16px", padding:"28px 24px", border:`1px solid ${border}`, fontFamily:"inherit" }}>
      <div style={{ marginBottom:"20px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"4px" }}>
          ...
          <h3 style={{ margin:0, fontSize:"18px", fontWeight:700, color:"#2a2a28" }}>Kalkulačka penetrace</h3>
        </div>
        <p style={{ margin:0, fontSize:"13px", color:muted }}>Spočítej kolik litrů penetrace (primeru) potřebuješ.</p>
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(140px, 1fr))", gap:"14px", marginBottom:"18px" }}>
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Plocha k penetraci (m²)</label>
        ...
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Vydatnost penetrace (m²/l)</label>
          <input type="number" min="0" step="0.5" value={vydatnost}
            onChange={(e) => setVydatnost(e.target.value)}
            style={inputCss}
            onFocus={e=>(e.target.style.borderColor=accent)}
            onBlur={e=>(e.target.style.borderColor=border)} />
          <span style={{ fontSize:"11px", color:muted, marginTop:"4px" }}>viz etiketa — savé povrchy: 6–8 m²/l</span>
        </div>
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Počet vrstev</label>
          <input type="number" min="0" step="1" value={vrstvy}
            onChange={(e) => setVrstvy(e.target.value)}
            style={inputCss}
            onFocus={e=>(e.target.style.borderColor=accent)}
            onBlur={e=>(e.target.style.borderColor=border)} />
          <span style={{ fontSize:"11px", color:muted, marginTop:"4px" }}>na savé/nové omítky doporučujeme 2</span>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"14px" }}>
        <div style={{ background:"#fff", border:`1px solid ${border}`, borderRadius:"10px", padding:"12px 14px" }}>
          ...
          <div style={{ fontSize:"20px", fontWeight:700, color:"#2a2a28" }}>{n(plocha).toFixed(1)} m²</div>
        </div>
        <div style={{ background:"#fff", border:`1px solid ${border}`, borderRadius:"10px", padding:"12px 14px" }}>
          <div style={{ fontSize:"10px", color:muted, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:"4px" }}>Litrů celkem</div>
          <div style={{ fontSize:"20px", fontWeight:700, color:"#2a2a28" }}>{litry.toFixed(1)} l</div>
        </div>
      </div>
      ...
      <div style={{ background:accent, borderRadius:"12px", padding:"20px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"12px" }}>
        <div>
          ...
          <div style={{ fontSize:"32px", fontWeight:800, color:"#fff", lineHeight:1.1 }}>{litryZ.toFixed(1)} litrů penetrace</div>
        </div>
      ...
      <div style={{ marginTop:"12px", background:"#fffbf0", border:"1px solid #e8d890", borderRadius:"8px", padding:"10px 14px", fontSize:"12px", color:"#7a6a20", display:"flex", gap:"8px" }}>
        <span>💡</span><span>Na savé nebo nové omítky (sádra, vápenopísek, porobeton) aplikujte 2 vrstvy. Bez penetrace barva prosakuje hlouběji a spotřeba vzroste o 20–40 %.</span>
      </div>
    </div>
  );
}
