...
export default function HnojivoCalculator() {
  const [plocha, setPlocha]   = useState("50");
  const [davka, setDavka]     = useState("30");
  const [pytlik, setPytlik]   = useState("5");
  ...
  const copyText = `Potřebujete ${pytlu} pytlíků hnojiva (${kg.toFixed(2)} kg) na ${n(plocha)} m². Spočíteno na DomovniGuru.cz`;
    ...
    <div style={{ background: bg, borderRadius:"16px", padding:"28px 24px", border:`1px solid ${border}`, fontFamily:"inherit" }}>
      <div style={{ marginBottom:"20px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"4px" }}>
          ...
          <h3 style={{ margin:0, fontSize:"18px", fontWeight:700, color:"#2a2a28" }}>Kalkulačka hnojiva</h3>
        </div>
        <p style={{ margin:0, fontSize:"13px", color:muted }}>Spočítej kolik kg hnojiva potřebuješ na záhon nebo trávník.</p>
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(140px, 1fr))", gap:"14px", marginBottom:"18px" }}>
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Plocha (m²)</label>
        ...
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Dávka hnojiva (g/m²)</label>
          <input type="number" min="0" step="1" value={davka}
            onChange={(e) => setDavka(e.target.value)}
            style={inputCss}
            onFocus={e=>(e.target.style.borderColor=accent)}
            onBlur={e=>(e.target.style.borderColor=border)} />
          <span style={{ fontSize:"11px", color:"#9a9a90", marginTop:"4px" }}>trávník: 30–40 g / záhony: 20–30 g</span>
        </div>
        <div style={{ display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"14px" }}>
        <div style={{ background:"#fff", border:`1px solid ${border}`, borderRadius:"10px", padding:"12px 14px" }}>
          <div style={{ fontSize:"10px", color:muted, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:"4px" }}>Celkem hnojiva</div>
      ...
      <div style={{ background:accent, borderRadius:"12px", padding:"20px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"12px" }}>
        <div>
          ...
          <div style={{ fontSize:"32px", fontWeight:800, color:"#fff", lineHeight:1.1 }}>{pytlu} pytlíků hnojiva</div>
          <div style={{ fontSize:"12px", color:"#b0b0a0", marginTop:"4px" }}>({kg.toFixed(2)} kg, pytlíky po {n(pytlik)} kg)</div>
        </div>
      ...
      <div style={{ marginTop:"12px", background:"#fffbf0", border:"1px solid #e8d890", borderRadius:"8px", padding:"10px 14px", fontSize:"12px", color:"#7a6a20", display:"flex", gap:"8px" }}>
        <span>💡</span><span>Trávník: 30–40 g/m², záhony: 20–30 g/m², ovocné stromy: 50–80 g/m² korunové projekce. Přehnojení škodí – vždy raději méně.</span>
      </div>
    </div>
  );
}
