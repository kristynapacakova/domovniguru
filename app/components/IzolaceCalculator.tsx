...
export default function IzolaceCalculator() {
  const [delka, setDelka]   = useState("8");
  const [sirka, setSirka]   = useState("6");
  const [baleni, setBaleni] = useState("5.76");
  const [rezerva, setRezerva] = useState("10");

  const n = (v: string) => parseFloat(v.replace(",", ".")) || 0;
  const plocha    = n(delka) * n(sirka);
  const r         = n(rezerva) / 100;
  const plochaRez = plocha * (1 + r);
  const baleniN   = n(baleni);
  const pocet     = baleniN > 0 ? Math.ceil(plochaRez / baleniN) : 0;
  const copyText  = `Potřebujete ${pocet} balení izolace (${plochaRez.toFixed(2)} m² včetně ${n(rezerva)}% rezervy na prořezy). Spočíteno na DomovniGuru.cz`;
    ...
    <div style={{ background: bg, borderRadius:"16px", padding:"28px 24px", border:`1px solid ${border}`, fontFamily:"inherit" }}>
      <div style={{ marginBottom:"20px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"4px" }}>
          ...
          <h3 style={{ margin:0, fontSize:"18px", fontWeight:700, color:"#2a2a28" }}>Kalkulačka izolace</h3>
        </div>
        <p style={{ margin:0, fontSize:"13px", color:muted }}>Spočítej potřebu tepelné nebo zvukové izolace s rezervou na prořezy.</p>
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(140px, 1fr))", gap:"14px", marginBottom:"18px" }}>
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Délka (m)</label>
        ...
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Šířka (m)</label>
        ...
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>m² / balení</label>
          <input type="number" min="0.1" step="0.1" value={baleni} onChange={e=>setBaleni(e.target.value)} style={inputCss} onFocus={e=>(e.target.style.borderColor=accent)} onBlur={e=>(e.target.style.borderColor=border)} />
          <span style={{ fontSize:"11px", color:"#9a9a90", marginTop:"4px" }}>viz etiketa balení</span>
        </div>
        <div style={{ display:"flex", flexDirection:"column" }}>
          <label style={labelCss}>Rezerva na prořez (%)</label>
          <input type="number" min="0" max="25" step="1" value={rezerva} onChange={e=>setRezerva(e.target.value)} style={inputCss} onFocus={e=>(e.target.style.borderColor=accent)} onBlur={e=>(e.target.style.borderColor=border)} />
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"14px" }}>
        <div style={{ background:"#fff", border:`1px solid ${border}`, borderRadius:"10px", padding:"12px 14px" }}>
          <div style={{ fontSize:"10px", color:muted, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:"4px" }}>Čistá plocha</div>
        ...
        <div style={{ background:"#fff", border:`1px solid ${border}`, borderRadius:"10px", padding:"12px 14px" }}>
          ...
          <div style={{ fontSize:"20px", fontWeight:700, color:"#2a2a28" }}>{plochaRez.toFixed(2)} m²</div>
        </div>
      </div>
      ...
      <div style={{ background:accent, borderRadius:"12px", padding:"20px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"12px" }}>
        <div>
          ...
          <div style={{ fontSize:"32px", fontWeight:800, color:"#fff", lineHeight:1.1 }}>{pocet} balení</div>
          <div style={{ fontSize:"12px", color:"#b0b0a0", marginTop:"4px" }}>izolace · {plochaRez.toFixed(2)} m² celkem</div>
        </div>
      ...
      <div style={{ marginTop:"12px", background:"#fffbf0", border:"1px solid #e8d890", borderRadius:"8px", padding:"10px 14px", fontSize:"12px", color:"#7a6a20", display:"flex", gap:"8px" }}>
        <span>💡</span><span>Na tepelných mostech (rohy, ostění) tloušťku nesnižuj — právě tam uniká nejvíce tepla.</span>
      </div>
    </div>
  );
}
