...
export default function LaminatCalculator() {
  ...
  const [sirka, setSirka]   = useState("4");
  ...
  const copyText  = `Potřebujete ${pocet} balení laminátu (${plochaRez.toFixed(2)} m² včetně ${n(rezerva)}% rezervy na prořez). Spočíteno na DomovniGuru.cz`;
    ...
    <div style={{ background: bg, borderRadius:"16px", padding:"28px 24px", border:`1px solid ${border}`, fontFamily:"inherit" }}>
      <div style={{ marginBottom:"20px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"4px" }}>
          ...
          <h3 style={{ margin:0, fontSize:"18px", fontWeight:700, color:"#2a2a28" }}>Kalkulačka laminátu</h3>
        </div>
        <p style={{ margin:0, fontSize:"13px", color:muted }}>Spočítej počet balení včetně rezervy na prořezy a dilataci.</p>
      </div>
      ...
      <div style={{ background:accent, borderRadius:"12px", padding:"20px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"12px" }}>
        <div>
          ...
          <div style={{ fontSize:"12px", color:"#b0b0a0", marginTop:"4px" }}>{plochaRez.toFixed(2)} m² celkem</div>
        </div>
      ...
      <div style={{ marginTop:"12px", background:"#fffbf0", border:"1px solid #e8d890", borderRadius:"8px", padding:"10px 14px", fontSize:"12px", color:"#7a6a20", display:"flex", gap:"8px" }}>
        <span>💡</span><span>Při pokládce na šikmo nebo ve vzoru přidej 15 %. Laminát nechej 48 h aklimatizovat v místnosti před pokládkou.</span>
      </div>
    </div>
  );
}
